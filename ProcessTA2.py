...
def AnalyzeStock(sno,stype,ai):
    ...
    df = cc.pd.read_csv(cc.FPATH+"/"+stype+"/"+sno+".csv",index_col=0) 
    ...
    df = df[:-10].copy()
    df.to_csv(cc.FOUTPATH+"/"+stype+"/P_"+sno+".csv",index=False)
    


def ProcessTA(stype,ai):

    snolist = list(map(lambda s: s.replace(".csv", ""), cc.os.listdir(cc.FPATH+"/"+stype)))
