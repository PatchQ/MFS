...
# 專案根目錄：不管在哪個工作目錄執行，永遠正確
PROJECT_ROOT = Path(__file__).resolve().parent.parent
IOPATH = PROJECT_ROOT / "SData" / "HKEX" / "IO"
...
def extract_data(odate):
    ...
    df['call_ratio'] = round(df['call_turnover'].astype(float) / df['call_deals'].astype(float).replace(0, float('nan')), 2)
    df['put_ratio']  = round(df['put_turnover'].astype(float)  / df['put_deals'].astype(float).replace(0, float('nan')),  2)

    # 原始欄位（未含新增的 prev/change）
    desired_order = [
        'series', 'month_num', 'month_abbr', 'year', 'strike',
        'call_deals', 'call_turnover',
        'call_ratio',
        'call_settle_price', 'call_price_change',
        'call_net', 'call_net_change', 'call_gross',
        'put_deals', 'put_turnover',
        'put_ratio',
        'put_settle_price', 'put_price_change',
        'put_net', 'put_net_change', 'put_gross',
    ]
    ...
    for op in oplist:
        ...
        tempdf = df_op_selected.loc[df_op_selected['series'] == op].copy()
...
if __name__ == "__main__":    
    ...
    parser = argparse.ArgumentParser(description="HKEX Index Options 下載與解析")
