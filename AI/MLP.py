...
def MLP(sno, stype, tdate):
    ...
    if len(train_data) > 500:
        ...
        model = make_pipeline(imputer, scaler, mlp)
...
def Process(stype,tdate):
    ...
    with executor:
        for tempdf in tqdm(executor.map(MLP,SLIST["sno"],SLIST["stype"],SLIST["tdate"],chunksize=1),total=len(SLIST)):                        
