...
print("=== HFH Signal Trace with Updated Params ===")
...
# Now manually trace through all conditions using the ACTUAL parameter values
...
# Use ACTUAL parameter values from the function
max_flat_pct = 0.12
min_flat_length = 4
min_strong_bullish = 2
max_body_deviation = 0.50  # Updated!
min_flat_body_ratio = 0.20  # Updated!
min_close_strength = 0.5
max_upper_wick = 0.35
min_volume_ratio = 1.2
vol_trend_threshold = 0.5  # Updated from 0.8 to 0.5
...
for i in range(250, min(1000, len(result))):
    ...
    if close_strength < min_close_strength:
        ...
        continue
    condition_stats['close_strength_ok'] += 1
    ...
    if upper_wick_ratio > max_upper_wick:
        ...
        continue
    condition_stats['upper_wick_ok'] += 1
    
    vol_ratio = volumes[i] / vol_ma20[i]
    if vol_ratio < min_volume_ratio:
        fail_reasons['volume'] += 1
        continue
    condition_stats['volume_ratio_ok'] += 1
    
    if i >= 3:
        vol_trend = volumes[i-2:i+1]
        vol_trend_ma = np.mean(volumes[max(0, i-5):i])
        if np.mean(vol_trend) < vol_trend_ma * vol_trend_threshold:
            fail_reasons['vol_trend'] += 1
            continue
    condition_stats['vol_trend_ok'] += 1
    ...
    if np.max(body_devs) > max_body_deviation:
        ...
        continue
    condition_stats['body_dev_ok'] += 1
    
    if np.min(flat_body_pcts[valid_mask]) < min_flat_body_ratio:
        fail_reasons['body_pct'] += 1
        continue
    condition_stats['body_pct_ok'] += 1
    
    condition_stats['final_signal'] += 1
    print(f"\n*** SIGNAL at {result.index[i]} (index {i}) ***")
    print(f"  Flat: {result.index[curr_start]} to {result.index[i]}, len={flat_len}")
    ...
    print(f"  Vol ratio: {vol_ratio:.3f}")
    print(f"  Body dev max: {np.max(body_devs):.3f}")
    print(f"  Min body pct: {np.min(flat_body_pcts[valid_mask]):.3f}")

print("\n=== Condition Stats ===")
for k, v in condition_stats.items():
    print(f"  {k}: {v}")

print("\n=== Failure Reasons (after pre_high OK) ===")
