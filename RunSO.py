...
def filterOption(filename, oyear, omonth, oday, strike):
    ...
    op = filename.split("_")[0]+"_"+filename.split("_")[1]
    filedate = filename.split("_")[2].replace(".csv", "")    
    df = cc.pd.read_csv(f"{cc.SOPATH}{op}/{filename}", index_col=0) 
...
def getStrike(op, oyear, omonth, oday, strike, sdate, edate):   
   ...
   filenamelist = []   
   templist = list(map(lambda s: s.replace(".csv", ""), cc.os.listdir(cc.SOPATH+"/"+op)))

   for tp in templist:
       filedate = tp.split("_")[2]       
   ...
   if len(resultdf)>0:
        if resultdf["lotgross"].any() or resultdf["lotnet"].any():
            if resultdf["lotnet"].any():
                resultdf.to_csv(f"{cc.SOPATH}/P_{op}/C_{op}{oyear}{omonth}{strike}.csv",index=False)
            else:
                resultdf.to_csv(f"{cc.SOPATH}/P_{op}/{op}{oyear}{omonth}{strike}.csv",index=False)
