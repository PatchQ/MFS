...
def find_resistance_test_pattern(symbol, period="1y", resistance_threshold=0.05, min_days_between_peaks=10):
    """
    找出股票三次測試同一高位的模式
...
def analyze_resistance_patterns(symbol, period="1y",resistance_threshold=0.05):
    """
    分析並顯示阻力位測試模式
    """
    result = find_resistance_test_pattern(symbol,period,resistance_threshold=resistance_threshold)
...
def visualize_resistance_pattern(symbol, period="1y", resistance_threshold=0.05, pattern_index=0):
    """
    可視化阻力位測試模式
    """
    result = find_resistance_test_pattern(symbol,period,resistance_threshold=resistance_threshold)
...
if __name__ == "__main__":
    ...
    # 使用簡化版本（更穩定）
    #result = simple_resistance_pattern(symbol,period=period,lookback_days=lookback_days,resistance_threshold=resistance_threshold)
    result = None
    ...
    if result is None:
        print("\n嘗試完整版本分析...")
        result = analyze_resistance_patterns(symbol,period=period,resistance_threshold=resistance_threshold)
        
        # 可視化第一個找到的模式
        if result and result['patterns']:
            visualize_resistance_pattern(symbol, period=period, pattern_index=0,resistance_threshold=resistance_threshold)