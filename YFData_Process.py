...
def CalData(sno):
    ...
    df["DC_1"] = df["Adj Close"].pct_change(periods=1).shift(periods=-1)
    df["DC_5"] = df["Adj Close"].pct_change(periods=5).shift(periods=-5)
    df["DC_10"] = df["Adj Close"].pct_change(periods=10).shift(periods=-10)
    #df["DC_10"] = (df["Adj Close"].shift(periods=-10).rolling(10).max() - df["Adj Close"])/df["Adj Close"]
    ...
    df["cond1"] = ((df["Adj Close"] > df["SMA_150"]) & (df["Adj Close"] > df["SMA_200"]))
    ...
    df["cond5"] = (df["Adj Close"] > df["SMA_50"])
    ...
    #df["cond10"]  = df["Volume"]*df["Adj Close"] >= 2000000
